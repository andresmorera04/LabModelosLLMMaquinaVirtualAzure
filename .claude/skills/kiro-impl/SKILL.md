...
**Iteration discipline**: Process exactly ONE sub-task (e.g., 1.1) per iteration. Do NOT batch multiple sub-tasks into a single subagent dispatch. Each iteration follows the full cycle: dispatch implementer → review → mark complete → re-read tasks.md → next. **Do NOT create git commits.** All commits are the sole responsibility of the developer.
...
- **APPROVED** → before marking the task `[x]` or making any success claim, apply `kiro-verify-completion` using fresh evidence from the current code state; then mark task `[x]` in tasks.md. **Do NOT create a git commit — all commits are the developer's responsibility.**
...
**f) Debug subagent** (triggered by BLOCKED, NEEDS_CONTEXT unresolved, or REJECTED after 2 remediation rounds):
...
- **No Automatic Commits**: NEVER create git commits, stage files, or push. All git operations are the sole responsibility of the developer
